...
Custom:EnabledAFK()
local _OpenCloseImageId = nil
local Open_Close = nil
local function SetOpenCloseImage(id)
	if id and id ~= "" and Open_Close then
		Open_Close.Image = "rbxassetid://" .. tostring(id)
	end
end
...
local Sibah2xZcu_Library = {}
Sibah2xZcu_Library.Unloaded = false
local NotifGui
...
function Sibah2xZcu_Library:SetNotification(Config)
	EnsureNotifGui()
...
function Sibah2xZcu_Library:CreateWindow(Config)
	...
	local Icon             = Config[6] or Config.Icon             or "rbxthumb://type=Asset&id=132281476392315&w=420&h=420"
	local IsVerified       = Config.isVerified                    or false
	local OpenCloseImageId = Config.OpenCloseImage                or nil
	if OpenCloseImageId then
		SetOpenCloseImage(OpenCloseImageId)
	end

	local KeyConfig = Config.KeySystem
	if KeyConfig and KeyConfig.Enabled then
		local KS_Title       = KeyConfig.Title       or Title or "Key System"
		local KS_Desc        = KeyConfig.Description or "Enter your key to continue."
		local KS_GetKeyLink  = KeyConfig.GetKeyLink  or nil
		local KS_Keys        = KeyConfig.Key         or {}
		local KS_ValidateUrl = KeyConfig.ValidateUrl or nil
		if type(KS_Keys) == "string" then KS_Keys = {KS_Keys} end

		local KS_W = 360
		local KS_H = KS_GetKeyLink and 196 or 170
		...
		local KS_ShadowHolder = Custom:Create("Frame", {
			BackgroundTransparency = 1,
			BorderSizePixel = 0,
			AnchorPoint = Vector2.new(0.5, 0.5),
			Position = UDim2.new(0.5, 0, 0.5, 0),
			Size = UDim2.new(0, KS_W, 0, KS_H),
			ZIndex = 2,
			Name = "KS_ShadowHolder",
		}, KeyGui)

		local KS_Main = Custom:Create("Frame", {
			AnchorPoint = Vector2.new(0.5, 0.5),
		...
		Custom:Create("UICorner", {}, KS_Main)
		Custom:Create("UIStroke", {Color = Color3.fromRGB(255, 255, 255), Thickness = 1.2, Transparency = 0.7}, KS_Main)

		local KS_Top = Custom:Create("Frame", {
		...
		Custom:Create("UICorner", {}, KS_Top)

		local KS_IconWidth = 0
		if Icon ~= "" then
			KS_IconWidth = 34
		...
		local KS_TitleLabel = Custom:Create("TextLabel", {
			Font = Enum.Font.GothamBold,
			Text = KS_Title,
			...
			BorderSizePixel = 0,
			ZIndex = 5,
			Position = UDim2.new(0, 10 + KS_IconWidth, 0, 0),
			Size = UDim2.new(1, -(10 + KS_IconWidth + 10), 1, 0),
		}, KS_Top)

		local KS_Sep = Custom:Create("TextLabel", {
			Font = Enum.Font.GothamBold,
			Text = "|",
			...
			Position = UDim2.new(0, 10 + KS_IconWidth + KS_TitleLabel.TextBounds.X + 8, 0, 0),
		}, KS_Top)
		...
		Custom:Create("TextLabel", {
			...
			Position = UDim2.new(0, KS_Sep.Position.X.Offset + 14, 0, 0),
		}, KS_Top)
		...
		local KS_InputContainer = Custom:Create("Frame", {
			BackgroundColor3 = Color3.fromRGB(255, 255, 255),
		...
		Custom:Create("UICorner", {CornerRadius = UDim.new(0, 6)}, KS_InputContainer)
		local KS_InputStroke = Custom:Create("UIStroke", {
			Color = Color3.fromRGB(70, 70, 70),
			Thickness = 1,
			Transparency = 0.4,
		}, KS_InputContainer)

		local KS_Input = Custom:Create("TextBox", {
			Font = Enum.Font.Gotham,
		...
		KS_Input.Focused:Connect(function()
			TweenService:Create(KS_InputStroke, TweenInfo.new(0.15), {Color = Custom.ColorRGB, Transparency = 0}):Play()
		end)
		KS_Input.FocusLost:Connect(function()
			TweenService:Create(KS_InputStroke, TweenInfo.new(0.15), {Color = Color3.fromRGB(70, 70, 70), Transparency = 0.4}):Play()
		end)

		local KS_Status = Custom:Create("TextLabel", {
			Font = Enum.Font.Gotham,
			Text = "",
		...
		if KS_GetKeyLink then
			local KS_GetKey = Custom:Create("TextButton", {
				Font = Enum.Font.GothamBold,
				Text = "Dapatkan Kunci",
				...
				BorderSizePixel = 0,
				ZIndex = 4,
				Position = UDim2.new(0, 16, 0, KS_BtnY),
				Size = UDim2.new(0.5, -20, 0, KS_BtnH),
				AutoButtonColor = false,
			}, KS_Main)
			Custom:Create("UICorner", {CornerRadius = UDim.new(0, 6)}, KS_GetKey)
			Custom:Create("UIStroke", {Color = Color3.fromRGB(70, 70, 70), Thickness = 1, Transparency = 0.3}, KS_GetKey)

			local KS_AccentColor = KeyConfig.AccentColor or Color3.fromRGB(88, 166, 255)
			local KS_Submit = Custom:Create("TextButton", {
				Font = Enum.Font.GothamBold,
				Text = "Submit",
				TextColor3 = Color3.fromRGB(255, 255, 255),
				TextSize = 12,
				BackgroundColor3 = KS_AccentColor,
				BorderSizePixel = 0,
				ZIndex = 4,
				Position = UDim2.new(0.5, 4, 0, KS_BtnY),
				Size = UDim2.new(0.5, -20, 0, KS_BtnH),
				AutoButtonColor = false,
			}, KS_Main)
			Custom:Create("UICorner", {CornerRadius = UDim.new(0, 6)}, KS_Submit)

			KS_GetKey.Activated:Connect(function()
				CircleClick(KS_GetKey, Player:GetMouse().X, Player:GetMouse().Y)
				pcall(function() if setclipboard then setclipboard(KS_GetKeyLink) end end)
				pcall(function() game:GetService("GuiService"):OpenBrowserWindow(KS_GetKeyLink) end)
				KS_Status.TextColor3 = Color3.fromRGB(88, 166, 255)
				KS_Status.Text = "Link opened!"
			end)

			local KS_Validating = false
			local function ShakeInput()
				local orig = KS_InputContainer.Position
				local ox = orig.X.Offset
				task.spawn(function()
					for i = 1, 6 do
						KS_InputContainer.Position = UDim2.new(orig.X.Scale, ox + (i % 2 == 0 and 5 or -5), orig.Y.Scale, orig.Y.Offset)
						task.wait(0.045)
					end
					KS_InputContainer.Position = orig
				end)
			end
			local function ValidateKey(InputKey)
				if KS_Validating then return end
				InputKey = InputKey:match("^%s*(.-)%s*$")
				if InputKey == "" then
					KS_Status.TextColor3 = Color3.fromRGB(255, 80, 80)
					KS_Status.Text = "Key cannot be empty!"
					ShakeInput(); return
				end
				for _, k in ipairs(KS_Keys) do
					if InputKey == k then
						KS_Status.TextColor3 = Color3.fromRGB(80, 220, 100)
						KS_Status.Text = "Key valid! Loading..."
						task.wait(0.6); KeyGui:Destroy(); return
					end
				end
				if KS_ValidateUrl then
					KS_Validating = true
					KS_Status.TextColor3 = Color3.fromRGB(170, 170, 170)
					KS_Status.Text = "Validating..."
					KS_Submit.Text = "..."
					task.spawn(function()
						local ok, result = pcall(function() return game:HttpGet(KS_ValidateUrl) end)
						KS_Validating = false; KS_Submit.Text = "Submit"
						if ok and result then
							for line in result:gmatch("[^\r\n]+") do
								line = line:match("^%s*(.-)%s*$")
								if line ~= "" and InputKey == line then
									KS_Status.TextColor3 = Color3.fromRGB(80, 220, 100)
									KS_Status.Text = "Key valid! Loading..."
									task.wait(0.6); KeyGui:Destroy(); return
								end
			...
			KS_Submit.Activated:Connect(function()
				CircleClick(KS_Submit, Player:GetMouse().X, Player:GetMouse().Y)
				ValidateKey(KS_Input.Text)
			end)
			KS_Input.FocusLost:Connect(function(enter) if enter then ValidateKey(KS_Input.Text) end end)
		else
			local KS_AccentColor = KeyConfig.AccentColor or Color3.fromRGB(88, 166, 255)
			local KS_Submit = Custom:Create("TextButton", {
				Font = Enum.Font.GothamBold,
		...
		MakeDraggable(KS_Top, KS_ShadowHolder)
		repeat task.wait(0.1) until not KeyGui or not KeyGui.Parent
		if KeyGui and KeyGui.Parent then KeyGui:Destroy() end
	end

	local Funcs = {}
	local SearchRegistry = {}
	local SibahGui = Custom:Create("ScreenGui", {
		ZIndexBehavior = Enum.ZIndexBehavior.Sibling
	}, Player.PlayerGui)
	local DropShadowHolder = Custom:Create("Frame", {
		...
		Position = UDim2.new(0, (SibahGui.AbsoluteSize.X // 2 - 400 // 2), 0, (SibahGui.AbsoluteSize.Y // 2 - 310 // 2))
	}, SibahGui)
	...
	if Icon ~= "" then
		IconWidth = 34
		local _iconInst = Custom:Create("ImageLabel", {
			Image = Icon,
			AnchorPoint = Vector2.new(0, 0.5),
			BackgroundTransparency = 1,
			BorderSizePixel = 0,
		...
		_TopIconRef = _iconInst
		TextLabel.Position = UDim2.new(0, 10 + IconWidth, 0, 0)
	end
	local VerifiedBadge = nil
	local VerifiedWidth = 0
	if IsVerified then
		VerifiedWidth = 20
		VerifiedBadge = Custom:Create("TextLabel", {
			Font = Enum.Font.GothamBold,
			Text = "",
			...
			BorderSizePixel = 0,
			AnchorPoint = Vector2.new(0, 0.5),
			Position = UDim2.new(0, 10 + IconWidth + TextLabel.TextBounds.X + 4, 0.5, 0),
	...
	local Separator = Custom:Create("TextLabel", {
		...
		BorderSizePixel = 0,
		Size = UDim2.new(0, 12, 1, 0),
		Position = UDim2.new(0, 10 + IconWidth + TextLabel.TextBounds.X + VerifiedWidth + 8, 0, 0),
		Name = "Separator"
	}, Top)
	...
	function Funcs:SetTitle(NewTitle)
		...
		if VerifiedBadge then
			VerifiedBadge.Position = UDim2.new(0, 10 + IconWidth + TextLabel.TextBounds.X + 4, 0.5, 0)
		end
		Separator.Position = UDim2.new(0, 10 + IconWidth + TextLabel.TextBounds.X + VerifiedWidth + 8, 0, 0)
	...
	function Funcs:SetColor(NewColor)
		if typeof(NewColor) ~= "Color3" then return end
		Custom.ColorRGB = NewColor
	end
	function Funcs:Notify(Text, Delay)
		Sibah2xZcu_Library:SetNotification({
			Content = Text,
			Delay   = Delay or 3,
		})
	end
	local _TopIconRef = nil
	function Funcs:SetIcon(NewId)
		if _TopIconRef then
			_TopIconRef.Image = "rbxassetid://" .. tostring(NewId)
		end
	end
	...
	ExitButton.Activated:Connect(function()
		...
		task.spawn(function()
			HideExitConfirm()
			if SibahGui then SibahGui:Destroy() end
			if not Sibah2xZcu_Library.Unloaded then Sibah2xZcu_Library.Unloaded = true end
		end)
	end)
	...
	function Tabs:CreateTab(Config)
		...
		function Sections:AddSection(Title, OpenSection)
			...
			function Item:AddButton(Config)
				...
				local BgTrans    = 0.935
				local TitleColor = Color3.fromRGB(231, 231, 231)
				...
				if Style == "Danger" then
					Custom:Create("UIStroke", {Color = Color3.fromRGB(255, 80, 80), Thickness = 1, Transparency = 0.5}, B)
				end
				Custom:Create("TextLabel", {
					Font = Enum.Font.GothamBold, Text = Title, TextColor3 = TitleColor,
					TextSize = 13, TextXAlignment = Enum.TextXAlignment.Left, TextYAlignment = Enum.TextYAlignment.Top,
					BackgroundTransparency = 0.999, BorderSizePixel = 0,
			...
			function Item:AddColorPicker(Config)
				local Title    = Config[1] or Config.Title    or ""
				local Default  = Config[2] or Config.Default  or Color3.fromRGB(255, 255, 255)
				local Callback = Config[3] or Config.Callback or function() end
				local FCP = {Value = Default}
				...
				Custom:Create("TextLabel", {
					...
					BackgroundTransparency = 0.999, BorderSizePixel = 0,
					Position = UDim2.new(0, 10, 0, 0), Size = UDim2.new(1, -160, 1, 0), Name = "CPTitle"
				}, CP)
				local Preview = Custom:Create("Frame", {
					...
					Size = UDim2.new(0, 22, 0, 22), Name = "ColorPreview"
				}, CP)
				Custom:Create("UICorner", {CornerRadius = UDim.new(0, 4)}, Preview)
				Custom:Create("UIStroke", {Color = Color3.fromRGB(255,255,255), Thickness = 1.2, Transparency = 0.6}, Preview)
				local Expanded = false
				local SliderH = Custom:Create("Frame", {
					BackgroundTransparency = 0.999, BorderSizePixel = 0,
					Position = UDim2.new(0, 0, 1, 2),
					Size = UDim2.new(1, 0, 0, 0),
					ClipsDescendants = true, Name = "CPSliders"
				}, CP)
				local function MakeHSVSlider(label, posY, defaultVal, color)
					local Track = Custom:Create("Frame", {
						BackgroundColor3 = Color3.fromRGB(255,255,255), BackgroundTransparency = 0.85,
						BorderSizePixel = 0, Position = UDim2.new(0, 10, 0, posY),
						Size = UDim2.new(1, -20, 0, 6), Name = label.."Track"
					}, SliderH)
					Custom:Create("UICorner", {CornerRadius = UDim.new(1,0)}, Track)
					local Fill = Custom:Create("Frame", {
						BackgroundColor3 = color, BorderSizePixel = 0,
						Size = UDim2.new(defaultVal, 0, 1, 0), Name = label.."Fill"
					}, Track)
					...
					local Knob = Custom:Create("Frame", {
						...
						BorderSizePixel = 0, Position = UDim2.new(defaultVal, 0, 0.5, 0),
						Size = UDim2.new(0, 10, 0, 10), Name = label.."Knob"
					}, Track)
					Custom:Create("UICorner", {CornerRadius = UDim.new(1,0)}, Knob)
					Custom:Create("TextLabel", {
						Font = Enum.Font.GothamBold, Text = label, TextColor3 = Color3.fromRGB(180,180,180),
						TextSize = 10, BackgroundTransparency = 1,
						Position = UDim2.new(0, -2, 0, posY - 13), Size = UDim2.new(0, 10, 0, 12)
					}, SliderH)
					return {Track = Track, Fill = Fill, Knob = Knob, Value = defaultVal}
				end
				local h0, s0, v0 = Color3.toHSV(Default)
				local SH = MakeHSVSlider("H", 18, h0, Color3.fromHSV(h0, 1, 1))
				local SS2 = MakeHSVSlider("S", 42, s0, Color3.fromRGB(180,180,180))
				local SV2 = MakeHSVSlider("V", 66, v0, Color3.fromRGB(255,255,255))
				local COLLAPSED_H = 35
				local EXPANDED_H  = 90
				local function UpdateColor()
					local c = Color3.fromHSV(SH.Value, SS2.Value, SV2.Value)
					FCP.Value = c
					Preview.BackgroundColor3 = c
					SH.Fill.BackgroundColor3 = Color3.fromHSV(SH.Value, 1, 1)
					Callback(c)
				end
				local function BindSlider(sData)
					local dragging = false
					sData.Track.InputBegan:Connect(function(i)
						if i.UserInputType == Enum.UserInputType.MouseButton1 or i.UserInputType == Enum.UserInputType.Touch then
							dragging = true
						end
					end)
					sData.Track.InputEnded:Connect(function(i)
						if i.UserInputType == Enum.UserInputType.MouseButton1 or i.UserInputType == Enum.UserInputType.Touch then
							dragging = false
						end
					end)
					UserInputService.InputChanged:Connect(function(i)
						if dragging and (i.UserInputType == Enum.UserInputType.MouseMovement or i.UserInputType == Enum.UserInputType.Touch) then
							local sc = math.clamp((i.Position.X - sData.Track.AbsolutePosition.X) / sData.Track.AbsoluteSize.X, 0, 1)
							sData.Value = sc
							sData.Fill.Size = UDim2.new(sc, 0, 1, 0)
							sData.Knob.Position = UDim2.new(sc, 0, 0.5, 0)
							UpdateColor()
						end
					end)
				end
				BindSlider(SH); BindSlider(SS2); BindSlider(SV2)
				local CPBtn = Custom:Create("TextButton", {
					Font = Enum.Font.SourceSans, Text = "", BackgroundTransparency = 0.999,
					BorderSizePixel = 0, Size = UDim2.new(1, 0, 0, 35), Name = "CPBtn"
				}, CP)
				CPBtn.Activated:Connect(function()
					CircleClick(CPBtn, Player:GetMouse().X, Player:GetMouse().Y)
					Expanded = not Expanded
					local newH = Expanded and EXPANDED_H or 0
					TweenService:Create(SliderH, TweenInfo.new(0.15), {Size = UDim2.new(1,0,0,newH)}):Play()
					TweenService:Create(CP, TweenInfo.new(0.15), {Size = UDim2.new(1,0,0, COLLAPSED_H + newH)}):Play()
					task.delay(0.2, UpdateSizeSection)
				end)
				function FCP:Set(NewColor)
					FCP.Value = NewColor
					Preview.BackgroundColor3 = NewColor
					local nh, ns, nv = Color3.toHSV(NewColor)
					SH.Value = nh; SS2.Value = ns; SV2.Value = nv
					SH.Fill.Size = UDim2.new(nh,0,1,0); SH.Knob.Position = UDim2.new(nh,0,0.5,0)
					SS2.Fill.Size = UDim2.new(ns,0,1,0); SS2.Knob.Position = UDim2.new(ns,0,0.5,0)
					SV2.Fill.Size = UDim2.new(nv,0,1,0); SV2.Knob.Position = UDim2.new(nv,0,0.5,0)
					Callback(NewColor)
				end
				UpdateColor()
				RegisterSearch(Title, CP, SearchSubtitle, SelectTab, EnsureSectionOpen, ScrolLayers)
