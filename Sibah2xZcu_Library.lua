...
function Sibah2xZcu_Library:CreateWindow(Config)
	...
	if KeyConfig and KeyConfig.Enabled then
		...
		local KS_TitleLabel = Custom:Create("TextLabel", {
			...
			Size = UDim2.new(1, -(10 + KS_IconWidth + 50), 1, 0),
		}, KS_Top)
		...
		local KS_Body = Custom:Create("Frame", {
			BackgroundTransparency = 1,
			BorderSizePixel = 0,
			ZIndex = 3,
		...
		KS_Input.FocusLost:Connect(function()
			TweenService:Create(KS_InputStroke, TweenInfo.new(0.15), {Color = Color3.fromRGB(80, 80, 80), Transparency = 0.5}):Play()
		end)
		...
		local KS_Submit = Custom:Create("TextButton", {
			...
			BorderSizePixel = 0,
			ZIndex = 4,
			Position = KS_GetKeyLink
				and UDim2.new(0.5, 4, 1, -14)
				or  UDim2.new(0, 16, 1, -14),
			Size = KS_GetKeyLink
				and UDim2.new(0.5, -20, 0, 30)
				or  UDim2.new(1, -32, 0, 30),
		...
		if KS_GetKeyLink then
			...
			Custom:Create("UIStroke", {Color = Color3.fromRGB(80, 80, 80), Thickness = 1, Transparency = 0.3}, KS_GetKey)
			KS_GetKey.Activated:Connect(function()
				CircleClick(KS_GetKey, Player:GetMouse().X, Player:GetMouse().Y)
				local ok, _ = pcall(function()
					if setclipboard then setclipboard(KS_GetKeyLink) end
				end)
				...
				KS_Status.Text = "Link copied & opened in browser!"
			end)
		end
		...
		local function ShakeInput()
			...
			task.spawn(function()
				for i = 1, 5 do
					KS_InputContainer.Position = UDim2.new(orig.X.Scale, ox + (i % 2 == 0 and 6 or -6), orig.Y.Scale, orig.Y.Offset)
					task.wait(0.05)
				end
		...
		local function ValidateKey(InputKey)
			...
			if KS_ValidateUrl then
				KS_Validating = true
				KS_Status.TextColor3 = Color3.fromRGB(180, 180, 180)
				KS_Status.Text = "Validating key..."
				KS_Submit.Text = "..."
				task.spawn(function()
					local ok, result = pcall(function()
						return game:HttpGet(KS_ValidateUrl)
					end)
					KS_Validating = false
					KS_Submit.Text = "Submit"
					...
					KS_Status.TextColor3 = Color3.fromRGB(255, 80, 80)
					KS_Status.Text = "Invalid key! Please try again."
					ShakeInput()
				end)
				return
			end
		...
		KS_Input.FocusLost:Connect(function(enterPressed)
			if enterPressed then ValidateKey(KS_Input.Text) end
		end)
